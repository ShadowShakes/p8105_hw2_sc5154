...
-   The two separate datasets denoted as `mr_trash_data` and
    `professor_trash_data` respectively record the information of trash
...
-   The combined dataset `combine_trash_data` has 641 rows and 15
    columns, with 547 rows of data from Mr. Trash Wheel and 94 rows of
    data from Professor Trash Wheel.
...
-   The second dataset `snp_data` contains 787 observations of 3
    variables related to Standard & Poor’s stock market index, and the
    `close` variable refers to the closing values of the stock index on
    the associated date.

-   The third dataset `unemployment_data` contains 810 observations of 3
...
-   The merged dataset `result_rmna_fte_data` that has `NA` removed
